...
function MeetThePersonPage() {
  ...
  const [meetPersons, setMeetPersons] = useState([]);
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const filteredFullScreenAd = fullScreenAdData
          .filter(
            (ad) => ad.status && ad.page_type?.toLowerCase() === "meet the person",
          )
          .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
          .slice(0, 1)
          .map((ad) => ({
        ...
        console.log("Meet The Person Error");
      } finally {
        setLoading(false);
      }
    };
...
export default MeetThePersonPage;
